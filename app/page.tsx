import Image from "next/image";
import Hero from "../assets/images/hero.png";
import image from "../assets/images/image.png";
import OurServices from "../components/landing/OurServices";
import WhatSetUsApart from "../components/landing/WhatSetUsApart";
import Trending from "../components/landing/Trending";
import Testimonials from "../components/landing/Testimonials";

export default function Home() {
  return (
    <div className="flex w-full bg-[#fbfbfb] flex-col ">
      <div className="w-full px-10 pt-20 relative">
        <Image src={Hero} alt="Hero image" className="w-full h-full object-cover rounded-t-xl" />

        {/* Overlay */}
        <div className="absolute inset-0 flex flex-col items-center justify-center px-10 top-0 ">

          {/* Hero text */}
          <div className="text-center text-white">
            <div className="text-sm font-semibold uppercase tracking-widest mb-3 text-[#c9a96e] font-montserrat">
             
              Luxury African Travel
            </div>
            <h1 className="text-6xl font-bold leading-tight font-raleway">
              Discover Africa,<br />
             <div className="relative  flex  flex-row justify-end ">


  <em className="absolute inset-0 flex items-center justify-center font-cormorant italic z-10 text-white">
    curated for you.
  </em>
  <div className="pl-48">
    <Image
    src={image}
    alt="Wave image"
    className="w-44 h-auto mt-10    justify-end object-cover"
  />
  </div>
</div>
            </h1>
          </div>



         
        </div>

<div className="absolute bottom-30 max-w-4xl w-full   left-1/2 transform -translate-x-1/2">
 <div className=" ">
          <div className="bg-white rounded-2xl shadow-xl flex items-center px-6 py-4 gap-2">

            {/* Where */}
            <div className="flex items-center gap-3 flex-1">
              <svg className="w-4 h-4 shrink-0" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M8 1.5C5.5 1.5 3.5 3.5 3.5 6C3.5 9.5 8 14 8 14C8 14 12.5 9.5 12.5 6C12.5 3.5 10.5 1.5 8 1.5Z" stroke="#c9a96e" strokeWidth="1.2" fill="none" />
                <circle cx="8" cy="6" r="1.5" stroke="#c9a96e" strokeWidth="1.2" fill="none" />
              </svg>
              <div className="flex flex-col">
                <span className="text-xs text-gray-400 font-open-sans">Where</span>
                <span className="text-sm text-gray-600 font-open-sans">Search destinations</span>
              </div>
            </div>

            <div className="w-px h-8 bg-gray-200" />

            {/* When */}
            <div className="flex items-center gap-3 flex-1">
              <svg className="w-4 h-4 shrink-0" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect x="2" y="3" width="12" height="11" rx="1.5" stroke="#c9a96e" strokeWidth="1.2" fill="none" />
                <path d="M5 1.5V4M11 1.5V4M2 7H14" stroke="#c9a96e" strokeWidth="1.2" strokeLinecap="round" />
              </svg>
              <div className="flex flex-col">
                <span className="text-xs text-gray-400 font-open-sans">When</span>
                <span className="text-sm text-gray-600 font-open-sans">February 05 ~ March 14</span>
              </div>
            </div>

            <div className="w-px h-8 bg-gray-200" />

            {/* Tour Type */}
            <div className="flex items-center gap-3 flex-1">
              <svg className="w-4 h-4 shrink-0" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M3 5H13M3 8H10M3 11H8" stroke="#c9a96e" strokeWidth="1.2" strokeLinecap="round" />
              </svg>
              <div className="flex flex-col">
                <span className="text-xs text-gray-400 font-open-sans">Tour Type</span>
                <span className="text-sm text-gray-600 font-open-sans">All tours</span>
              </div>
            </div>

            <button className="bg-primary text-white px-6 py-3 rounded-xl text-sm font-semibold font-montserrat hover:opacity-90 transition-opacity shrink-0">
              Search
            </button>
          </div>
        </div>

</div>
        {/* Search bar */}
       
      </div>


<Trending/>
      <div className="w-full px-20 mt-25 py-10">


        <div className="text-3xl font-bold font-raleway">
          Why Choose AzuraTravel?
        </div>

        <div className="mt-4 text-base font-open-sans  max-w-5xl">
          At the heart of Azuratravels lies a commitment to providing access to extraordinary experiences that celebrate Africa's rich tapestry of culture, 
lifestyle, and heritage. We address a critical market need: the demand for structured, high-quality, and culturally authentic travel experiences.
        </div>

        <div className="mt-20 flex flex-row gap-10">

          <div className="flex flex-col gap-4">
            <div className="flex px-10">
              <svg fill="#bd8f3a" height="35px" width="35px" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 396.21 396.21" xmlSpace="preserve" stroke="#bd8f3a"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <path d="M375.925,276.23c-0.911-2.865-3.547-4.791-6.557-4.791c-1.48,0-2.895,0.469-4.089,1.354l-21.416,15.89 c-4.819,3.577-8.326,8.74-9.873,14.539l-7.503,28.136c-1.918,7.195,1.833,14.748,8.725,17.566l3.925,1.605 c1.797,0.734,3.686,1.107,5.617,1.107c5.616,0,10.688-3.108,13.235-8.113l17.241-33.867c4.496-8.832,5.317-18.897,2.313-28.342 L375.925,276.23z M368.102,306.028l-17.241,33.867c-1.176,2.308-3.516,3.742-6.106,3.742c-0.887,0-1.758-0.172-2.588-0.512 l-3.925-1.605c-3.179-1.3-4.908-4.783-4.023-8.102l7.503-28.135c1.082-4.059,3.536-7.673,6.91-10.176l20.15-14.949l1.14,3.582 C372.284,291.168,371.637,299.083,368.102,306.028z"></path> <path d="M321.558,257.12c-2.672-16.517,3.949-33.059,17.278-43.171l20.13-15.27c10.044-7.62,16.855-18.641,19.179-31.033 l1.014-5.408c1.196-6.381-0.496-12.904-4.642-17.899c-4.146-4.995-10.245-7.859-16.736-7.859c-9.542,0-21.853-6.85-26.882-14.958 l-29.111-46.926C286.065,49.252,262.116,29.3,234.289,18.39c-1.419-0.555-2.908-0.836-4.426-0.836 c-4.263,0-8.266,2.275-10.448,5.938l-3.395,5.699c-2.692,4.519-7.634,7.326-12.897,7.327c-2.06,0-4.069-0.42-5.973-1.247 l-10.337-4.494c-4.02-1.748-7.065-5.153-8.355-9.343c-1.29-4.19-0.688-8.719,1.653-12.426c0.96-1.519,1.076-3.398,0.313-5.027 c-0.762-1.624-2.276-2.733-4.054-2.966c-5.952-0.778-11.938-1.112-17.852-0.991l-35.143,0.718 C95.334,1.313,70.017,16.035,55.651,40.121L23.165,94.582c-9.115,15.281-8.591,34.697,1.335,49.465l15.983,23.78 c6.192,9.213,16.513,14.713,27.608,14.713c1.416,0,2.848-0.092,4.255-0.272l51.29-6.603c6.941-0.896,13.911,1.707,18.58,6.895 c3.832,4.256,5.74,9.667,5.436,15.343c-1.141,1.647-1.686,3.604-1.518,5.563c0.213,2.49,1.563,4.784,3.7,6.292l10.182,7.188 c11.845,8.361,16.821,23.774,12.102,37.483l-3.368,9.783c-5.875,17.066-5.579,35.757,0.832,52.629l23.074,60.723 c3.908,10.284,13.534,17.237,24.524,17.714l20.845,0.906c0.4,0.018,0.799,0.026,1.197,0.026c6.873,0,13.455-2.551,18.536-7.182 l17.277-15.754c6.892-6.284,11.187-14.857,12.094-24.142l1.147-11.764c0.894-9.166,5.873-17.244,13.659-22.162l5.313-3.355 c12.766-8.063,19.456-22.902,17.044-37.807L321.558,257.12z M123.538,8.74l35.143-0.718c4.222-0.087,8.476,0.076,12.744,0.483 c-1.929,4.836-2.172,10.221-0.613,15.283c1.978,6.424,6.648,11.645,12.812,14.324l10.337,4.494 c2.916,1.269,5.998,1.911,9.163,1.911c8.066-0.001,15.642-4.305,19.77-11.233l3.396-5.699c0.746-1.254,2.116-2.032,3.575-2.032 c0.517,0,1.025,0.097,1.567,0.308c26.169,10.26,48.741,29.065,63.559,52.95l12.147,19.581l-38.85,69.625l-26.725-43.829 l1.426-24.25c0.087-1.479-0.65-2.885-1.917-3.653l-41.997-25.499c-1.79-1.086-4.115-0.608-5.331,1.094l-5.441,7.617l-13.389-7.303 c-1.251-0.683-2.771-0.646-3.991,0.093l-39.575,24.027L64.354,41.319C77.551,21.345,99.423,9.232,123.538,8.74z M126.421,167.486 c-1.262,0-2.542,0.082-3.806,0.244l-51.29,6.602c-1.069,0.138-2.157,0.207-3.233,0.207c-8.427,0-16.266-4.178-20.969-11.176 l-15.983-23.78c-8.208-12.212-8.642-28.268-1.104-40.904l30.104-50.469l68.321,56.086c1.311,1.076,3.159,1.21,4.614,0.327 l40.028-24.304l12.334,6.729l-1.4,57.426l-34.843,33.946c-0.332-0.413-0.676-0.818-1.032-1.213 C142.601,171.029,134.676,167.486,126.421,167.486z M176.316,266.815l3.368-9.783c5.87-17.052-0.32-36.224-15.053-46.622 l-10.183-7.188c-0.203-0.144-0.331-0.307-0.342-0.439c-0.012-0.132,0.087-0.314,0.263-0.49c0.647-0.649,1.054-1.499,1.15-2.41 c0.52-4.944-0.243-9.89-2.123-14.388l37.391-36.428c0.749-0.73,1.183-1.723,1.208-2.768l1.47-60.268l4.629-6.48l36.765,22.322 l-1.357,23.096c-0.048,0.813,0.154,1.621,0.578,2.316l36.727,60.23l-12.821,78.349l-31.84,6.922 c-1.834,0.399-3.145,2.02-3.15,3.896l-0.07,22.608l-49.783-0.974C171.775,287.777,172.819,276.976,176.316,266.815z M269.647,367.362l-17.276,15.754c-3.813,3.475-8.801,5.31-13.996,5.075l-20.845-0.906c-7.795-0.339-14.622-5.27-17.394-12.564 l-23.074-60.723c-0.953-2.509-1.742-5.065-2.393-7.65l67.366,1.318l38.073,31.058l-0.94,9.63 C278.455,355.665,275.074,362.415,269.647,367.362z M302.98,305.088l-5.314,3.355c-7.872,4.972-13.474,12.491-16.009,21.221 l-35.635-29.07c-0.693-0.565-1.556-0.882-2.45-0.899l-12.647-0.247l0.06-19.526l31.357-6.817c1.604-0.349,2.833-1.642,3.098-3.263 l13.5-82.496c0.154-0.943-0.034-1.911-0.532-2.727l-5.338-8.754l38.89-69.695l12.141,19.57 c6.52,10.509,21.314,18.741,33.681,18.741c4.103,0,7.96,1.811,10.581,4.969c2.62,3.157,3.689,7.282,2.934,11.316l-1.014,5.408 c-1.956,10.436-7.692,19.717-16.15,26.134l-20.13,15.27c-15.692,11.904-23.486,31.378-20.34,50.821l2.737,16.925 C318.297,287.056,313.03,298.74,302.98,305.088z"></path> </g> </g></svg>
            </div>

            <div className="flex flex-col ">
              <span className="font-bold mb-4 font-raleway">Cultural Authenticity</span>

              <p className=" font-open-sans">
                Curated journeys that go deep into Africa's heritage, art, music, 
and community life — not surface-level tourism.

              </p>

              <p>

              </p>

            </div>

          </div>

   <div className="flex flex-col gap-4">
                    <div className="flex px-10">
<svg fill="#bd8f3a" version="1.1" height="30px"width="30px"  id="Layer_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 253" xmlSpace="preserve" stroke="#bd8f3a"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M102.416,148.646l-8.293-21.305l0.027-0.031h26.819L102.416,148.646z M90.278,131.592l-14.922,17.16h21.602L90.278,131.592z M109.2,148.752h37.599l-18.644-21.441h-0.31L109.2,148.752z M161.968,127.447l-0.118-0.136h-26.821l18.624,21.418L161.968,127.447z M180.644,148.752l-14.833-17.057l-6.664,17.057H180.644z M157.121,154.112l-22.96,58.768l46.616-58.768H157.121z M75.223,154.112 l46.646,58.81l-22.892-58.81H75.223z M127.973,214.311l23.579-60.199h-47.009L127.973,214.311z M2,69 c0,13.678,9.625,25.302,22,29.576V233H2v18h252v-18h-22V98.554c12.89-3.945,21.699-15.396,22-29.554v-8H2V69z M65.29,68.346 c0,6.477,6.755,31.47,31.727,31.47c21.689,0,31.202-19.615,31.202-31.47c0,11.052,7.41,31.447,31.464,31.447 c21.733,0,31.363-20.999,31.363-31.447c0,14.425,9.726,26.416,22.954,30.154V233H42V98.594C55.402,94.966,65.29,82.895,65.29,68.346 z M222.832,22H223V2H34v20L2,54h252L222.832,22z"></path> </g></svg>
            </div>

            <div className="flex flex-col ">
              <span className="font-bold mb-4 font-raleway">Luxury Positioning </span>

              <p className=" font-open-sans">
                Curated journeys that go deep into Africa's heritage, art, music, 
and community life — not surface-level tourism.

              </p>

              <p>

              </p>

            </div>

          </div>

   <div className="flex flex-col gap-4">
            <div className="flex px-10">
              <svg fill="#bd8f3a" height="30px" width="30px" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384.747 384.747" xmlSpace="preserve" stroke="#bd8f3a"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <path d="M275.684,169.341c-4.237,0-8.222,1.65-11.219,4.646l-31.752,31.753l-26.459,0.002l6.435-6.435 c1.563-1.563,1.563-4.095,0-5.657c-1.563-1.561-4.096-1.561-5.656,0l-12.093,12.093l-51.544,0.004 c-1.062,0-2.078,0.421-2.828,1.171l-9.197,9.197c-0.915,0.915-1.331,2.216-1.116,3.492c0.214,1.276,1.033,2.37,2.196,2.935 l56.174,27.288l-8.19,8.19l-35.617-0.006c-0.001,0-0.001,0-0.001,0c-1.061,0-2.078,0.421-2.828,1.171l-4.68,4.679 c-0.915,0.915-1.331,2.216-1.116,3.492c0.214,1.276,1.033,2.37,2.196,2.935l23.136,11.238c-0.639,1.885-1.12,3.825-1.383,5.806 c-0.455,3.412,0.583,6.853,2.848,9.439c2.271,2.594,5.539,4.082,8.967,4.082c0.001,0,0.001,0,0.002,0 c0.53,0,1.067-0.036,1.599-0.106c1.956-0.261,3.872-0.733,5.735-1.359l10.888,22.68c0.535,1.114,1.554,1.918,2.761,2.179 c0.281,0.061,0.564,0.09,0.846,0.09c0.931,0,1.842-0.325,2.569-0.934l5.092-4.266c0.907-0.76,1.431-1.883,1.431-3.066 l-0.006-35.617l7.635-7.635l25.614,53.379c0.535,1.114,1.555,1.919,2.764,2.18c0.279,0.06,0.562,0.09,0.843,0.09 c0.932,0,1.845-0.326,2.572-0.937l9.998-8.397c0.905-0.76,1.428-1.881,1.428-3.063l0.004-51.003l12.362-12.363 c1.563-1.563,1.563-4.095,0-5.657c-1.563-1.562-4.096-1.562-5.656,0l-6.705,6.705l0.002-24.16l33.171-33.171 c2.997-2.997,4.648-6.981,4.648-11.219c0-4.238-1.651-8.222-4.648-11.219C283.907,170.991,279.923,169.341,275.684,169.341z M141,217.8l4.054-4.054l79.659-0.006l-30.102,30.102L141,217.8z M147.895,266.014l24.541,0.004l-3.478,3.478 c-1.476,1.476-2.773,3.112-3.917,4.846L147.895,266.014z M194.878,313.371l-8.391-17.48c1.758-1.153,3.416-2.464,4.909-3.957 l3.478-3.478L194.878,313.371z M245.725,314.21l-4.622,3.882l-24.6-51.266l29.228-29.229L245.725,314.21z M281.247,190.768 l-85.201,85.202c-0.002,0.002-0.003,0.003-0.005,0.005l-10.302,10.302c-3.555,3.555-8.257,5.878-13.24,6.543 c-0.182,0.024-0.363,0.036-0.542,0.036c-1.122,0-2.196-0.493-2.948-1.352c-0.757-0.864-1.09-1.97-0.938-3.112 c0.664-4.982,2.988-9.684,6.543-13.239l95.508-95.509c1.486-1.486,3.462-2.304,5.563-2.304c2.102,0,4.076,0.818,5.563,2.304 c1.485,1.486,2.305,3.461,2.305,5.562S282.733,189.283,281.247,190.768z"></path> <path d="M334.74,72.061c3.526-24.712-1.233-44.415-13.859-57.041c-10.244-10.243-24.75-15.296-43.274-15.009 c-17.02,0.262-36.57,5.048-58.114,14.209c-6.769-0.739-13.616-1.127-20.524-1.127c-49.637,0-96.302,19.329-131.399,54.428 c-41.397,41.397-59.129,97.627-53.221,151.741c-9.214,21.617-14.026,41.23-14.29,58.299c-0.285,18.471,4.764,33.031,15.008,43.275 c9.928,9.927,24.227,15.005,41.905,15.005c4.798,0,9.859-0.396,15.135-1.149c34.533,32.321,79.331,50.055,126.862,50.055 c49.637,0,96.302-19.33,131.399-54.427C401.329,259.358,402.779,144.818,334.74,72.061z M315.224,20.677 c9.754,9.754,13.952,24.897,12.319,44.088c-25.874-24.825-57.699-41.324-92.21-48.134C271.148,3.975,299.833,5.287,315.224,20.677z M20.721,315.179c-15.422-15.423-16.706-44.193-3.967-80.11c6.636,33.718,22.656,65.934,48.055,92.422 C45.614,329.126,30.472,324.929,20.721,315.179z M324.711,324.663c-33.587,33.587-78.244,52.085-125.743,52.085 c-43.455,0-84.526-15.491-116.924-43.846c19.281-4.231,40.979-12.989,63.301-25.596c1.924-1.086,2.602-3.526,1.516-5.45 c-1.086-1.923-3.525-2.603-5.449-1.516c-23.849,13.469-46.831,22.365-66.639,25.833c-0.515-0.503-1.036-0.999-1.547-1.51 c-69.335-69.335-69.335-182.151,0-251.486c33.587-33.587,78.244-52.084,125.743-52.084c47.499,0,92.156,18.497,125.743,52.084 c0.511,0.511,1.007,1.033,1.511,1.55c-3.729,21.292-13.657,46.063-28.827,71.815c-1.121,1.903-0.487,4.355,1.416,5.477 c0.638,0.376,1.337,0.554,2.026,0.554c1.37,0,2.705-0.705,3.45-1.97c14.285-24.248,24.091-47.773,28.657-68.601 C393.915,151.774,391.179,258.195,324.711,324.663z"></path> </g> </g></svg>

            </div>

            <div className="flex flex-col ">
              <span className="font-bold mb-4 font-raleway">Diaspora Bridge </span>

              <p className="font-open-sans">
                Uniquely positioned to connect the global African diaspora to 
their roots through meaningful, immersive experiences.

              </p>

              <p>

              </p>

            </div>

          </div>

   <div className="flex flex-col gap-4">
            <div className="flex px-10">
<svg width="30" height="30" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
<g clipPath="url(#clip0_2103_6020)">
<path d="M38.7675 31.7463V55.0716C38.7675 56.0691 37.6453 56.6926 36.7725 56.0691L30.0394 51.2063L23.3063 56.0691C22.4335 56.6926 21.3113 56.0691 21.3113 55.0716V31.7463H38.7675Z" fill="white"/>
<path d="M30.0393 38.7375C39.6801 38.7375 47.4955 30.9221 47.4955 21.2813C47.4955 11.6404 39.6801 3.82501 30.0393 3.82501C20.3985 3.82501 12.583 11.6404 12.583 21.2813C12.583 30.9221 20.3985 38.7375 30.0393 38.7375Z" fill="#FDF0EA"/>
<path d="M31.2864 12.8025L32.5333 16.6678C32.658 17.1665 33.1567 17.5406 33.7802 17.5406H37.7702C39.0171 17.5406 39.5158 19.0369 38.5183 19.785L35.2764 22.1541C34.7777 22.5281 34.653 23.0269 34.7777 23.5256L36.0245 27.3909C36.3986 28.5131 35.027 29.5106 34.1542 28.7625L30.9124 26.3934C30.4136 26.0194 29.9149 26.0194 29.4161 26.3934L26.1742 28.7625C25.1767 29.5106 23.9298 28.5131 24.3039 27.3909L25.5508 23.5256C25.6755 23.0269 25.5508 22.4035 25.052 22.1541L21.8102 19.785C20.8127 19.0369 21.3114 17.5406 22.5583 17.5406H26.5483C27.0471 17.5406 27.5458 17.1665 27.7952 16.6678L29.0421 12.8025C29.1667 11.6803 30.9124 11.6803 31.2864 12.8025Z" fill="white"/>
<path d="M13.2931 27.5104C13.9833 27.5104 14.5429 26.9508 14.5429 26.2605C14.5429 25.5703 13.9833 25.0107 13.2931 25.0107C12.6028 25.0107 12.0432 25.5703 12.0432 26.2605C12.0432 26.9508 12.6028 27.5104 13.2931 27.5104Z" fill="#BD8F3A"/>
<path fillRule="evenodd" clipRule="evenodd" d="M20.2563 37.3073C17.535 35.5864 15.2876 33.1852 13.7543 30.3413C13.4267 29.7347 13.6543 28.9756 14.2621 28.6479C14.8699 28.3203 15.629 28.5466 15.9567 29.1544C18.6968 34.2357 24.067 37.6912 30.2401 37.6912C39.1908 37.6912 46.4569 30.4251 46.4569 21.4756C46.4569 17.3823 44.9374 13.6417 42.4311 10.7865C41.9759 10.2688 42.0285 9.47712 42.5462 9.02189C43.0652 8.56666 43.8569 8.61795 44.3121 9.13696C47.2036 12.4324 48.9582 16.7508 48.9582 21.4756C48.9582 28.1365 45.4702 33.9894 40.2238 37.3073V50.25C40.2238 50.9403 39.6635 51.5006 38.9732 51.5006C38.2828 51.5006 37.7226 50.9403 37.7226 50.25V38.6355C35.4314 39.6372 32.9001 40.1925 30.2401 40.1925C27.58 40.1925 25.0487 39.6372 22.7576 38.6355V55.2863C22.7576 55.2788 22.7701 55.2687 22.7764 55.2662L29.5085 50.4051C29.9449 50.0887 30.5352 50.0887 30.9717 50.4051C30.9717 50.4051 37.71 55.2713 37.7038 55.2675C37.715 55.275 37.7226 55.2988 37.7226 55.2863V54.0019C37.7226 53.3115 38.2828 52.7513 38.9732 52.7513C39.6635 52.7513 40.2238 53.3115 40.2238 54.0019V55.2863C40.2238 57.2798 37.9939 58.5479 36.2506 57.3023L36.2456 57.2985L30.2401 52.9614L24.2346 57.2985L24.2296 57.3023C22.4862 58.5479 20.2563 57.2798 20.2563 55.2863V37.3073ZM14.0657 22.6474C14.1145 23.3353 13.5955 23.9344 12.9076 23.9831C12.2186 24.0332 11.6195 23.5142 11.5707 22.8251C11.5382 22.3798 11.522 21.9296 11.522 21.4756C11.522 11.1442 19.9099 2.75751 30.2401 2.75751C34.2271 2.75751 37.9252 4.00691 40.9629 6.13547C41.5282 6.53067 41.6658 7.31106 41.2693 7.87634C40.8729 8.44162 40.0925 8.57919 39.5272 8.18399C36.8959 6.34057 33.693 5.25876 30.2401 5.25876C21.2893 5.25876 14.0232 12.5249 14.0232 21.4756C14.0232 21.8696 14.0382 22.2597 14.0657 22.6474ZM26.707 16.4819C26.7283 16.4644 26.7984 16.4056 26.8459 16.3456C26.8459 16.3456 28.019 12.7075 28.019 12.7125C28.179 11.7557 28.8556 11.1642 29.6885 10.9666C30.8128 10.699 32.2286 11.2568 32.6763 12.6012L33.9244 16.4719C33.9419 16.4769 33.9657 16.4819 33.9832 16.4819H37.9752C39.2158 16.4819 40.0687 17.2485 40.3789 18.179C40.689 19.1094 40.4664 20.2337 39.4684 20.9829C39.4684 20.9829 36.2281 23.3503 36.2206 23.3566C36.2056 23.3703 36.1943 23.3853 36.1931 23.4041C36.1931 23.4041 37.4187 27.2047 37.4149 27.1935C37.7913 28.3203 37.3123 29.3921 36.4982 29.9861C35.6565 30.6014 34.4972 30.6965 33.5867 29.9473L30.3689 27.5962L30.3651 27.5924L30.3601 27.5962L27.1122 29.9686C25.0887 31.4731 22.5562 29.4671 23.3104 27.2047C23.3104 27.2047 24.5385 23.3991 24.5385 23.3941C24.5397 23.3891 24.5397 23.3841 24.5397 23.3791L21.2618 20.9829L21.2556 20.9791C20.2626 20.2338 20.0412 19.1094 20.3514 18.179C20.6615 17.2485 21.5132 16.4819 22.7538 16.4819H26.707ZM22.7638 18.9831C22.9489 19.1194 25.9217 21.2905 25.9167 21.288C26.8684 21.8346 27.211 23.0276 26.9622 24.0244L26.9522 24.0644L25.7178 27.8913C26.2393 27.5099 28.8756 25.5827 28.8756 25.5827C29.3684 25.2162 29.8711 25.0587 30.3651 25.0587C30.8579 25.0587 31.3606 25.2162 31.8546 25.5827L35.0125 27.8901L33.7781 24.0644L33.7681 24.0244C33.5255 23.0527 33.7581 22.0772 34.7361 21.343C34.7361 21.343 37.6888 19.1857 37.9652 18.9831H33.9832C32.7476 18.9831 31.8009 18.189 31.5345 17.211L30.3651 13.5842L29.1558 17.3335L29.1133 17.4186C28.648 18.3478 27.6763 18.9831 26.7471 18.9831H22.7638Z" fill="#BD8F3A"/>
</g>
<defs>
<clipPath id="clip0_2103_6020">
<rect width="60" height="60" fill="white"/>
</clipPath>
</defs>
</svg>

            </div>

            <div className="flex flex-col ">
              <span className="font-bold mb-4 font-raleway">Experience-Driven</span>

              <p className=" font-open-sans">
                Appealing to individuals who value exclusivity and cultural 
immersion over conventional, mass-market travel packages.

              </p>

              <p>

              </p>

            </div>

          </div>


        </div>

      </div>

      <OurServices/>
      <WhatSetUsApart/>
      <Testimonials/>
      
    </div>
  );
}
