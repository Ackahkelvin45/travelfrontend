...
export default function TourOverview({ description, highlights, included, notIncluded }: TourOverviewProps) {
  return (
    <section className="py-6 border-b border-gray-100">
      ...
      <p className="text-sm text-gray-500 font-open-sans leading-relaxed mb-5">{description}</p>

      <h3 className="text-base font-bold font-raleway text-text-primary mb-3">Tour Highlights</h3>
      <ul className="flex flex-col gap-2 mb-6">
        {highlights.map((item, i) => (
          <li key={i} className="flex items-start gap-2 text-sm text-gray-500 font-open-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2">
        {included.map((item, i) => (
          <div key={i} className="flex items-center gap-2 text-sm text-gray-500 font-open-sans">
